...
.silk-login-footer{color:#64748b !important;font-size:.75rem;text-align:center;margin-top:1.25rem;}
    ...
    with col_center:
        ...
        if st.button("🚀 Enter Industrial Platform (Demo Access)", use_container_width=True, key="demo_auth_btn"):
            ...
            # Write signed cookie so demo session persists across refreshes
            cookie_value = _create_auth_cookie_value(demo_user_info, demo_role)
            if cookie_value:
                _log.info("Demo Access: auth cookie created")
                _inject_cookie_and_redirect(cookie_value, "/")
                # _inject_cookie_and_redirect calls st.stop() — code below won't run
            else:
                # COOKIE_SECRET not configured — fall back to session-only demo access
                _log.info("Demo Access: no COOKIE_SECRET, using session-only mode")
                st.rerun()
...
def render_sidebar_user_profile():
    ...
    if st.sidebar.button("🚪 Sign Out", use_container_width=True):
        ...
        # Expire the auth cookie and redirect to login page
        # _inject_clear_cookie_and_redirect calls st.stop()
        _inject_clear_cookie_and_redirect("/")
