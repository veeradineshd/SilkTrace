...
def render_login_screen():
    ...
    # Build the action area HTML (Google button or standby notice)
    ...
    # Single st.markdown call — CSS + full HTML in one block so styles are never sandboxed
    st.markdown(f"""<style>
...
.silk-login-wrapper{{background:rgba(17,24,39,.72);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.1);border-radius:24px;padding:2.5rem 2.25rem 2rem;box-shadow:0 25px 60px -15px rgba(0,0,0,.6),inset 0 1px 1px rgba(255,255,255,.15);text-align:center;margin-bottom:1.25rem;}}
...
.silk-login-tagline{{color:#94a3b8 !important;font-size:.95rem;line-height:1.5;margin-bottom:1.75rem;font-weight:400;}}
...
.silk-google-btn{{display:flex;align-items:center;justify-content:center;gap:12px;width:100%;background-color:#fff;color:#1f2937 !important;text-decoration:none !important;padding:12px 24px;border-radius:12px;font-weight:600;font-size:.95rem;box-shadow:0 4px 16px rgba(0,0,0,.25);transition:all .2s cubic-bezier(.4,0,.2,1);margin-bottom:.75rem;border:1px solid #e5e7eb;}}
...
.silk-login-footer{{color:#64748b !important;font-size:.75rem;text-align:center;margin-top:1.25rem;}}
    ...
    with col_center:
        st.markdown(
            ...
            f'<div class="silk-login-logo">&#129525;</div>'
            ...
            f'</div>'
            f'{action_html}'
            f'<p class="silk-login-footer">Protected by SilkTrace Enterprise Security &nbsp;&#8226;&nbsp; SOC2 / OIDC Compliant</p>'
            f'</div>',
